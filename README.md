...
一套部署于工业产线的金属带材(金属片)表面缺陷视觉检测软件:3 台海康工业相机硬触发同步采图,**传统视觉算法 + YOLO-TensorRT 推理**双引擎并行判定 OK/NG,判定结果通过**相机 IO 频闪 + PLC Modbus TCP** 输出分拣信号,同时完成缺陷标注、存图、SQLite 落库与统计报表。采图→信号输出全链路 **< 150 ms**。
